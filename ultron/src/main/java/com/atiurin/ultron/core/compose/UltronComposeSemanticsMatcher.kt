...
open class UltronComposeSemanticsMatcher(
    ...
    fun withResultHandler(handler: (ComposeOperationResult<UltronComposeOperation>) -> Unit) =
        UltronComposeSemanticsMatcher(matcher, useUnmergedTree, timeoutMs, handler)

    fun withTimeout(timeoutMs: Long) = UltronComposeSemanticsMatcher(matcher, useUnmergedTree, timeoutMs, resultHandler)
    fun withUseUnmergedTree(value: Boolean) = UltronComposeSemanticsMatcher(matcher, value, timeoutMs, resultHandler)
    ...
    open fun getUltronComposeInteraction() = UltronComposeSemanticsNodeInteraction(getComposeRule().onNode(matcher, useUnmergedTree), timeoutMs, resultHandler)
}